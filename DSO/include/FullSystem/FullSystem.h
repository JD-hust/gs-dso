...
public:
	...
	void estimateDepthArange(ImmaturePoint* impt, FrameHessian* newFrame);
	float calculateNCC(std::vector<float> vICur, std::vector<float> vIRef);
	...
	GSMap* gsmapptr;
	void setgsmap(GSMap* gsmap);
	SE3 firstpose;
	void setFirstPose(SE3 pose);
	...
	// back-end时间
	std::vector<double> back_prepare_times;	
	std::vector<double> back_optimize_times;
	std::vector<double> back_takedepth_times;
	std::vector<double> depth_render_times;
	std::vector<double> depth_making_times;
	std::vector<double> back_after_1_times;
	std::vector<double> back_after_2_times;
	std::vector<double> nonkeyframe_times;
...
private:
	...
	void takeDepthMap(FrameHessian* fh, CalibHessian* HCalib);
