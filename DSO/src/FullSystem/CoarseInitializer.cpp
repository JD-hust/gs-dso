...
//#include "util/nanoflann.h" //2024.11.14 dj zhushi 
...
//[ ***step 3*** ] 在选出的像素中, 添加点信息
		for(int y=patternPadding+1;y<hl-patternPadding-2;y++)	//patternPadding : setting = 2
		for(int x=patternPadding+1;x<wl-patternPadding-2;x++)
		{
			...
			if((lvl!=0 && statusMapB[x+y*wl]) || (lvl==0 && statusMap[x+y*wl] != 0))
			{
				...
				// 2019.11.07 yzk
				pl[nl].mdepth = firstFrame->dDepth[lvl][x + y*w[lvl]];
...
// 2019.11.15 yzk
void CoarseInitializer::setFirstRGBD(CalibHessian* HCalib, FrameHessian* newFrameHessian)
{
	//[ ***step 1*** ] 计算图像每层的内参
	makeK(HCalib);
	firstFrame = newFrameHessian;
...
void CoarseInitializer::makeNN()
{
	...
	for(int lvl=0;lvl<pyrLevelsUsed;lvl++)
	{
		...
		for(int i=0;i<npts;i++)
		{
			...
			// 使用建立的KDtree, 来查询最近邻
			indexes[lvl]->findNeighbors(resultSet, (float*)&pt, nanoflann::SearchParameters());
			int myidx=0;
			float sumDF = 0;
			//* 给每个点的neighbours赋值
			for(int k=0;k<nn;k++)
			...
			if(lvl < pyrLevelsUsed-1 )
			{
				...
				indexes[lvl+1]->findNeighbors(resultSet1, (float*)&pt, nanoflann::SearchParameters());
