...
void FullSystem::setgsmap(GSMap* gsmap){
	gsmapptr = gsmap;
}

void FullSystem::setFirstPose(SE3 pose){
	firstpose = pose;
}
...
void FullSystem::printResult(std::string file)
{
	...
	// 判断路径是否存在
	if (std::filesystem::exists(std::filesystem::path(file).parent_path()) == false)
	{
...
//[ ***step 5*** ] 此时shell在跟踪阶段, 没人使用, 设置值
	...
	fh->shell->camToTrackingRef = lastF_2_fh.inverse(); // 当前帧到参考帧的位姿
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id )
{
	...
	if(!initialized)
	{
		...
		{
			// 处理第一帧
			fh->shell->poseValid = true;
			fh->shell->camToWorld = SE3(); // firstpose

			// render depth map first time
			auto t1 = std::chrono::high_resolution_clock::now();
			takeDepthMap(fh, &Hcalib);
			auto t2 = std::chrono::high_resolution_clock::now();
			auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
			std::cout << "takeDepthMap time for the first time is : " << duration << "ms" << std::endl;

			coarseInitializer->setFirstRGBD(&Hcalib, fh);
			initialized = true;
		}
		return;
	}
...
//[ ***step 6*** ] 判断是否插入关键帧
		...
		auto t2 = std::chrono::high_resolution_clock::now();
		auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
		tracking_times.push_back(duration);
...
void FullSystem::makeNonKeyFrame( FrameHessian* fh)
{
	...
	auto t2 = std::chrono::high_resolution_clock::now();
	auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
	nonkeyframe_times.push_back(duration);
}
...
//[ ***step 6*** ] 激活所有关键帧上的部分未成熟点(构造新的残差)
	...
	auto t2 = std::chrono::high_resolution_clock::now();
	auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
	back_prepare_times.push_back(duration);
...
//[ ***step 7*** ] 对滑窗内的关键帧进行优化(说的轻松, 里面好多问题)
	...
	back_optimize_times.push_back(duration);
	t1 = std::chrono::high_resolution_clock::now();
...
//[ ***step 9*** ] 标记删除和边缘化的点, 并删除&边缘化
	...
	t2 = std::chrono::high_resolution_clock::now();
	duration = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
	back_after_1_times.push_back(duration);
	t1 = std::chrono::high_resolution_clock::now();
...
//[ ***step 11*** ] 边缘化掉关键帧
	...
	t2 = std::chrono::high_resolution_clock::now();
	duration = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count();
	back_after_2_times.push_back(duration);
...
//@ 从GSMap中获取深度图
void FullSystem::takeDepthMap(FrameHessian* newFrame, CalibHessian* HCalib){
	...
	SE3 camToWorld = firstpose * newFrame->shell->camToWorld;
	// 数据类型转换：SE3->Tensor
	// 位姿类型转换：opencv->opengl
	Eigen::Matrix3d rotation = camToWorld.rotationMatrix().cast<double>();
	Eigen::Vector3d translation = camToWorld.translation().cast<double>();
	std::vector<double> vec(rotation.data(), rotation.data() + rotation.size());
	std::vector<double> vec1(translation.data(), translation.data() + translation.size());
	torch::Tensor R = torch::tensor(vec, torch::kFloat32).reshape({3, 3}).t(); // 注意：按列存储改为按行存储
	torch::Tensor T = torch::tensor(vec1, torch::kFloat32).reshape({3});

	R = torch::matmul(R, torch::diag(torch::tensor({1.0f, -1.0f, -1.0f}, R.device())));

	cam.camToWorld = torch::eye(4, torch::kFloat32);
	cam.camToWorld.index_put_({Slice(0, 3), Slice(0, 3)}, R);
	cam.camToWorld.index_put_({Slice(0, 3), 3}, T);
	
	{
	std::lock_guard<std::mutex> lock(gsmapptr->camQueueMutex);
	gsmapptr->cameraQueue.push(cam);
	gsmapptr->conditionVar.notify_one(); // 告知gsmap::run线程开始处理
	}

	{
		std::unique_lock<std::mutex> lock(gsmapptr->depthQueueMutex);
		gsmapptr->depthConditionVar.wait(lock, [this] { return !gsmapptr->depthQueue.empty(); });

		torch::Tensor depth = gsmapptr->depthQueue.front();
		cv::Mat depthcolor = depthTensorToImage(depth.detach().cpu(), newFrame->shell->incoming_id);
		gsmapptr->depthQueue.pop();

		torch::Tensor depth_1d = depth.detach().cpu().flatten();
		float* depthData = depth_1d.data_ptr<float>();

		auto stop = std::chrono::high_resolution_clock::now();  
		auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(stop - start).count(); 
		depth_render_times.push_back(duration);

		newFrame->makeDepthImages(depthData);

		auto stop2 = std::chrono::high_resolution_clock::now();  
		auto duration2 = std::chrono::duration_cast<std::chrono::milliseconds>(stop2 - stop).count(); 
		depth_making_times.push_back(duration2);
...
void FullSystem::makeNewTraces(FrameHessian* newFrame, float* gtDepth)
{
	...
	for(int x=patternPadding+1;x<wG[0]-patternPadding-2;x++)
	{
		...
		float idepth_rgbd = 1.0 / newFrame->dDepth[0][i];
		impt->idepth_min = 1.0 * idepth_rgbd;
		impt->idepth_max = 1.0 * idepth_rgbd;
		*/
		if(newFrame->dDepth[0][i] > 0)
		{
		    // impt->idepth_fromSensor = 1.0 / newFrame->dDepth[0][i]; 
			// impt->idepth_fromSensor = -1.0;
			impt->idepth_min = 1.0 / (newFrame->dDepth[0][i] * 1.2);
			impt->idepth_max = 1.0 / (newFrame->dDepth[0][i] * 0.8);
	    }
	    else
	    {
	    	impt->idepth_fromSensor = -1.0;
...
void FullSystem::estimateDepthArange(ImmaturePoint* impt, FrameHessian* newFrame)
{
	...
	SE3 Twc = newFrame->shell->camToWorld;
	SE3 Twl = refKeyFrame->shell->camToWorld;

	Mat33f Rwc = Twc.rotationMatrix().cast<float>();
	Vec3f twc = Twc.translation().cast<float>();

	Mat33f Rwl = Twl.rotationMatrix().cast<float>();
	Vec3f twl = Twl.translation().cast<float>();
    ...
    for(float dTemp = 2.0; dTemp < 8; dTemp = dTemp + 0.05) // Scene5,7,8: 2-5  Scene6: 2-8
    {

    	std::vector<float> vICur;
	    std::vector<float> vIRef;

        // method 1: 3D NCC
        /*
    	for(int i = u - step; i <= u + step; i++)
	    {
		    for(int j = v - step; j <= v + step; j++)
		    {
		    	//std::cout << "here-1" << std::endl;
			    float itensentityCur = newFrame->dI[j * wG[0] + i][0];
			    vICur.push_back(itensentityCur);

			    Vec3f Pc(i, j, 1.0);
			    Vec3f Pw = Rwc * K.inverse() * Pc * dTemp + twc;
			    Vec3f Pl = Rwl.inverse() * Pw - Rwl.inverse() * twl;

			    Vec3f Pl_normal = Pl / Pl(2, 0);
			    Pl_camera = K * Pl_normal;

			    if((int)(Pl_camera(1, 0) * wG[0] + Pl_camera(0, 0)) > wG[0] * hG[0] || (int)(Pl_camera(1, 0) * wG[0] + Pl_camera(0, 0)) < 0)
			    {
			    	fflag = true;
			    	break;
			    }

			    if((int)Pl_camera(0, 0) < 0 || (int)Pl_camera(0, 0) > wG[0] - 1)
	    	    {
			    	fflag = true;
			    	break;
			    }

	            if((int)Pl_camera(1, 0) < 0 || (int)Pl_camera(1, 0) > hG[0] - 1)
	    	    {
			    	fflag = true;
			    	break;
			    }

			    float itensentityRef = refKeyFrame->dI[(int)(Pl_normal(1, 0) * wG[0] + Pl_normal(0, 0))][0];
			    vIRef.push_back(itensentityRef);
		    }
	    ...
	    */
		...
		if((int)(Pl_camera(1, 0) * wG[0] + Pl_camera(0, 0)) > wG[0] * hG[0] - step || (int)(Pl_camera(1, 0) * wG[0] + Pl_camera(0, 0)) < 0 + step)
	    {
			continue;
	    }

	    if((int)Pl_camera(0, 0) - step < 0 || (int)Pl_camera(0, 0) + step > wG[0] - 1)
	    	continue;

	    if((int)Pl_camera(1, 0) - step < 0 || (int)Pl_camera(1, 0) + step > hG[0] - 1)
	    	continue;
	    ...
	    for(int i = (int)Pl_camera(0, 0) - step; i <= (int)Pl_camera(0, 0) + step; i++)
	    {
		    for(int j = (int)Pl_camera(1, 0) - step; j <= (int)Pl_camera(1, 0) + step; j++)
		    {
			    float itensentityRef = refKeyFrame->dI[(int)(j * wG[0] + i)][0];
			    vIRef.push_back(itensentityRef);
		    }
	    }
	    // method 2: 2D NCC

	    float tempNCC = calculateNCC(vICur, vIRef);
	    ...
	    else if(tempNCC < maxNCC - 0.0001 && tempNCC > secMaxNCC)
	    {
	    	secMaxNCC = tempNCC;
	    	secMaxd = dTemp;
	    	uSecMax = Pl_camera(0, 0);
	    	vSecMax = Pl_camera(1, 0);
	    }
    }

    if(maxNCC >= 0.9)  // Scene5,7,8: 0.9
    {
    	impt->idepth_min = 1 / (1.5 * maxd);  // Scene5,6,7,8: 1.5
        impt->idepth_max = 1 / (0.5 * maxd);  // Scene5,6,7,8: 0.5
    }
    else
    {
    	impt->ck = true;
    ...
    /*
    float pixelDistance = sqrt((uMax - uSecMax) * (uMax - uSecMax) + (vMax - vSecMax) * (vMax - vSecMax));
    if(pixelDistance < 10)
    {
    	impt->idepth_min = 1 / (1.5 * maxd);
    	impt->idepth_max = 1 / (0.5 * secMaxd);
    }
    else
    {
    	impt->idepth_min = 1 / (1.5 * maxd);
    	impt->idepth_max = 1 / (0.5 * maxd);
    }
    */
}

float FullSystem::calculateNCC(std::vector<float> vICur, std::vector<float> vIRef)
{
	float molecule = 0.0;
	...
	for(int i = 0; i < vICur.size(); i++)
	{
		molecule = molecule + vICur[i] * vIRef[i];
		temp1 = temp1 + vICur[i] * vICur[i];
		temp2 = temp2 + vIRef[i] * vIRef[i];
	}
