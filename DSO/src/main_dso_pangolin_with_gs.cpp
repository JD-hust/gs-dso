...
int main( int argc, char** argv )
{
	...
	// GSModule
	std::string inputPLY = resultPath + std::string("/debug.ply");
	std::string inputCFG = resultPath + std::string("/config.json");

	auto start_loadGS = std::chrono::high_resolution_clock::now();
	GSMap gsmap(inputPLY, inputCFG);
	...
	fullSystem->setgsmap(&gsmap);
	fullSystem->setFirstPose(reader->getFirstpose()); 
	...
	std::thread runthread([&]() { 
		...
		fullSystem->printResult(resultPath+std::string("result.txt"));
