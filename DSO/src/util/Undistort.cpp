...
// 2019.11.07 yzk
template<typename T>
ImageAndExposure* Undistort::transformDepthImage(const MinimalImage<T>* image_raw, float exposure, double timestamp, float factor) const
{
	if(image_raw->w != wOrg || image_raw->h != hOrg)
	{
		printf("Undistort::undistort: wrong image size (%d %d instead of %d %d) \n", image_raw->w, image_raw->h, w, h);
...
//[ ***step 1*** ] 去除光度参数的影响
	...
	float* data = dataArray;
	for(int i = 0; i < w * h; i++)
	{
		data[i] = image_raw->data[i];
	}

	memcpy(result->image, data, sizeof(float)*w*h);
	*/
	// 2019.12.05 yzk
	...
	if (!passthrough)
	{
		...
		memcpy(m.data, data, sizeof(float)*w*h);
		m.convertTo(m, CV_16UC1, 1.0);
		cv::imshow("in_data", m);
		*/
		// 2019.12.04 yzk
...
template ImageAndExposure* Undistort::transformDepthImage<unsigned short>(const MinimalImage<unsigned short>* image_raw, float exposure, double timestamp, float factor) const;
// 2019.11.07 yzk
