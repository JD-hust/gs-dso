...
int main(int argc, char *argv[]){
    ...
    options.add_options()
        ...
        ("resolution-schedule", "Double the image resolution every these many steps", cxxopts::value<int>()->default_value("3000")) // 被缩放的图像按照这个步长增加分辨率，直到达到原始分辨率 
    ...
    try{
        ...
        Camera *valCam = std::get<1>(t); // 指向验证相机,无则为nullptr
        ...
        int imageSize = -1;
        for (size_t step = 1; step <= numIters; step++){ // 迭代训练，每次迭代训练一个相机
            Camera& cam = cams[ camsIter.next() ];

            model.optimizersZeroGrad(); // 优化器梯度清零
            torch::Tensor rgb;
            rgb = model.forward(cam, step, false); // before: torch::Tensor rgb = model.forward(cam, step);
            torch::Tensor gt = cam.getImage(model.getDownscaleFactor(step));
            gt = gt.to(device);

            torch::Tensor mainLoss = model.mainLoss(rgb, gt, ssimWeight);
            mainLoss = mainLoss.requires_grad_(true);
            mainLoss.backward(); 
        ...
        model.save(outputScene + std::string("splat.ply"));
        model.saveConfig(outputScene + std::string("config.json"));

        model.savemydebugPly(outputScene + std::string("debug.ply")); 

        // Validate
        if (valCam != nullptr){
            torch::Tensor rgb = model.forward(*valCam, numIters, false);
