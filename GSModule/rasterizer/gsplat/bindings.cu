...
// cuda代码，调用cuda函数实现光栅化
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor> 
rasterize_forward_tensor(
    const std::tuple<int, int, int> tile_bounds, // 瓦片的数量（x,y,1)
    const std::tuple<int, int, int> block,    // blocK的大小(x,y,1)
    const std::tuple<int, int, int> img_size, // 图像的尺寸(w,h)
    ...
    rasterize_forward_2<<<tile_bounds_dim3, block_dim3>>>(
            tile_bounds_dim3,
            img_size_dim3,
    ...
    if (depth_flag) 
      out_depth = torch::split(out_img,1,2)[0];
