...
__global__ void rasterize_forward_f_depth(
    const dim3 tile_bounds, // tile 的大小
    const dim3 img_size,  // 图像大小
    const int32_t* __restrict__ gaussian_ids_sorted,
    const int2* __restrict__ tile_bins,// 每一个 tile 的高斯分布范围
    const float2* __restrict__ xys,    // 高斯分布的二维均值坐标
    const float3* __restrict__ conics, // 高斯分布的协方差矩阵
    const float* __restrict__ depths, // 每一个高斯的深度
    const float* __restrict__ opacities,
    float* __restrict__ final_Ts,  // 最后的透射率
    int* __restrict__ final_index,  
    float* __restrict__ out_depth, // 像素的深度
    const float3& __restrict__ background
) {
    // each thread draws one pixel, but also timeshares caching gaussians in a
    // shared tile
    // 每个线程处理一个像素，同时共享缓存高斯分布
    auto block = cg::this_thread_block();
    int32_t tile_id =
        block.group_index().y * tile_bounds.x + block.group_index().x; // 获取当前 tile 的索引,blockIdx.x + blockIdx.y * gridDim.x
    unsigned i =
        block.group_index().y * block.group_dim().y + block.thread_index().y;// 获取当前线程的索引(像素的y索引)
    unsigned j =
        block.group_index().x * block.group_dim().x + block.thread_index().x;// 获取当前线程的索引(像素的x索引)

    float px = (float)j;
    float py = (float)i;
    int32_t pix_id = i * img_size.x + j; // 计算当前线程处理的像素位置 (i, j) 和像素 ID pix_id。
    ...
    // first collect gaussians between range.x and range.y in batches
    // which gaussians to look through in this tile
    int2 range = tile_bins[tile_id]; // 获取当前 tile 的高斯分布范围,tile_id 为当前 tile的索引
    int num_batches = (range.y - range.x + BLOCK_SIZE - 1) / BLOCK_SIZE; // 计算当前 tile 的高斯分布范围的高斯分布数量
    ...
    // current visibility left to render
    float T = 1.f;   // 初始化当前像素的透射率
    // index of most recent gaussian to write to this thread's pixel
    int cur_idx = 0; // 初始化当前高斯分布索引

    // collect and process batches of gaussians
    // each thread loads one gaussian at a time before rasterizing its
    // designated pixel
    int tr = block.thread_rank(); // 获取当前线程的索引(高斯的索引)
    float pix_out = 0.f;
    for (int b = 0; b < num_batches; ++b) {
        // resync all threads before beginning next batch
        // end early if entire tile is done
        ...
        if (idx < range.y) {
            ...
            const float opac = opacities[g_id]; 
            xy_opacity_batch[tr] = {xy.x, xy.y, opac};
            conic_batch[tr] = conics[g_id]; // 获取当前高斯分布的透明度、二维均值坐标、协方差矩阵
        }
        ...
        for (int t = 0; (t < batch_size) && !done; ++t) {
            ...
            const float sigma = 0.5f * (conic.x * delta.x * delta.x +
                                        conic.z * delta.y * delta.y) +
                                conic.y * delta.x * delta.y;
            const float alpha = min(0.999f, opac * __expf(-sigma)); // 计算当前高斯分布的透明度
            if (sigma < 0.f || alpha < 1.f / 255.f) {
                continue;
            }

            const float next_T = T * (1.f - alpha); // 计算当前像素的透射率
            if (next_T <= 1e-4f) { // this pixel is done
                // we want to render the last gaussian that contributes and note
                // that here idx > range.x so we don't underflow
                done = true;
            ...
            int32_t g = id_batch[t];
            const float vis = alpha * T;
            const float c = depths[g];
            pix_out = pix_out + c * vis;
            T = next_T;
            cur_idx = batch_start + t;
        }
    }
    ...
    if (inside) {
        ...
        final_index[pix_id] =
            cur_idx; // index of in bin of last gaussian in this pixel
        float final_depth;
        final_depth = pix_out; // + T * background_depth(需要定义一个很大的背景深度)
        out_depth[pix_id] = final_depth;
    }
}

__global__ void rasterize_forward_2(
    const dim3 tile_bounds,
    const dim3 img_size,
    const int32_t* __restrict__ gaussian_ids_sorted,
    const int2* __restrict__ tile_bins,
    ...
    float* __restrict__ final_Ts,
    int* __restrict__ final_index,
    float3* __restrict__ out_img, // depth_flag for depth render
    const float3& __restrict__ background,
    const bool depth_flag
) {
    // each thread draws one pixel, but also timeshares caching gaussians in a
    // shared tile
    ...
    if (inside) {
        ...
        final_index[pix_id] =
            cur_idx; // index of in bin of last gaussian in this pixel
        if(depth_flag) {
            out_img[pix_id].x = pix_out.x + T * background.x;
        } else {
            float3 final_color;
            final_color.x = pix_out.x + T * background.x;
            final_color.y = pix_out.y + T * background.y;
