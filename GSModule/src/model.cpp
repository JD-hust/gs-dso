...
torch::Tensor Model::forward(Camera& cam, int step, bool depth_flag){
    ...
    if (device == torch::kCPU){
        ...
        cov2d = p[3];       // 协方差矩阵
        camDepths = p[4];   // 相机深度（指点投影后相机坐标系下的深度（尺度归一化后的值））
    }else{
...
void Model::afterTrain(int step){
    ...
    if (step < stopSplitAt){ // step < 5000
        torch::Tensor visibleMask = (radii > 0).flatten(); // 可视高斯的掩码
        // xys是高斯的2D坐标
...
void Model::savePly(const std::string &filename){
    // 检查是否filename路径目录与文件是否存在，不存在则创建
    if(!fs::exists(filename)){
        fs::create_directories(std::filesystem::path(filename).parent_path());
    }
...
void Model::savemydebugPly(const std::string &filename){
    std::ofstream o(filename, std::ios::binary);
    int numPoints = means.size(0);
...
void Model::saveConfig(const std::string &filename){
    nlohmann::json config;
    config["numDownscales"] = numDownscales;
    config["resolutionSchedule"] = resolutionSchedule;
    config["shDegree"] = shDegree;
    config["shDegreeInterval"] = shDegreeInterval;
    ...
    config["densifySizeThresh"] = densifySizeThresh;
    config["stopScreenSizeAt"] = stopScreenSizeAt;
    config["splitScreenSize"] = splitScreenSize;
    ...
    if (outputFile.is_open()) {
        ...
        outputFile.close();
        std::cout << "JSON data written to x.json successfully!" << std::endl;
    } else {
...
void Model::saveDebugPly(const std::string &filename){
    ...
    torch::Tensor meansCpu = keepCrs ? (means.cpu() / scale) + translation : means.cpu(); // 是否保持坐标系，如果保持坐标系，坐标系转换到世界坐标系；否则，坐标系转换到相机坐标系
    torch::Tensor rgbsCpu = (sh2rgb(featuresDc.cpu()) * 255.0f).toType(torch::kUInt8);
