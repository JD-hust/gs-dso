...
PointSet *fastPlyReadPointSet(const std::string &filename) {
    ...
    while (line != "end_header") {
        ...
        if (hasHeader(line, "red") || hasHeader(line, "diffuse_red")) {
            hasColors = true;
            redIdx = c;
        }
        if (hasHeader(line, "green") || hasHeader(line, "diffuse_green")) {
            hasColors = true;
            greenIdx = c;
        }
        if (hasHeader(line, "blue") || hasHeader(line, "diffuse_blue")) {
            hasColors = true;
            blueIdx = c;
        }
