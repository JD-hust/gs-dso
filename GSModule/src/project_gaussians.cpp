...
variable_list ProjectGaussians::forward(AutogradContext *ctx, 
                ...
                int imgHeight,
                int imgWidth,
                TileBounds tileBounds, // 瓦片边界
                float clipThresh 
            ){
    
    int numPoints = means.size(0);
    std::cout << "means_x_max: " << means.index({"...", 0}).max().item<float>() << " " << "means_x_min: " << means.index({"...", 0}).min().item<float>() << std::endl;
    std::cout << "means_y_max: " << means.index({"...", 1}).max().item<float>() << " " << "means_y_min: " << means.index({"...", 1}).min().item<float>() << std::endl;
    std::cout << "means_z_max: " << means.index({"...", 2}).max().item<float>() << " " << "means_z_min: " << means.index({"...", 2}).min().item<float>() << std::endl;
